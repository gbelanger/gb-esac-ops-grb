...
ao="16"
grbProposals="1640020, 1640025, 1640033"
grbProposalsAndPIs="1640020 (Gotz), 1640025 (Savchenko), 1640033 (Martin-Carrillo)"
note="Note that the PIs of GRB proposals (Gotz, Savchenko, and Martin-Carrillo) are granted access to the data of all GRB events but for different purposes that do not overlap in their scientific aims."

#  Extract the date and time of the GRB from the GCN circular
gcnNo=$(egrep NUMBER $1 | cut -d":" -f2 | awk '{print $1}')
gcn_dateAndTime=$(egrep DATE $1 | cut -d":" -f2,3,4 | awk '{print $1,$2}')
gcn_date=$(echo $gcn_dateAndTime | cut -d" " -f1)
#echo $gcn_dateAndTime
#echo $gcn_date
...
UT=$(egrep UT $1 | head -1)
echo $UT
grb_h=$(echo $UT | cut -d":" -f1 | cut -d" " -f4)
grb_m=$(echo $UT | cut -d":" -f2)
grb_s=$(echo $UT | cut -d":" -f3 | cut -d" " -f1)
grb_day=$(echo $UT | cut -d":" -f3 | cut -d" " -f5 | cut -d"," -f1)
...
if [[ ${flux_val} == "" ]] ; then
    flux_val=$(egrep 'ph/cm2/s' $1 | cut -d" " -f1)
fi
...
dist=$(calc.pl nint\($angDist\))
flx=$(calc.pl nint\($flux_val\))

dataRights="1640020 (Gotz) is granted data rights for polarization studies with IBIS, 1640025 (Savchenko) is granted access to the data of +\/- 1 day around the GRB for follow-up of multi-messenger transients, and 1640033 (Martin-Carrillo) is granted data rights on all instruments for GRB studies excluding those based on polarization with IBIS."

###   if flux is between 0.7 and 10
#if [[ $flx -ge 1 ]] && [[ $flx -lt 10 ]] ; then
#    dataRights="1540032 (Pozanenko) is granted data rights on all instruments."
#fi

src=$(echo $source | sed s/"\/"/"-"/g)
sed s/"{source}"/"$src"/g email_template.txt | sed s/"{ao}"/"$ao"/g | sed s/"{grbProposals}"/"$grbProposals"/g | sed s/"{grbProposalsAndPIs}"/"$grbProposalsAndPIs"/g | sed s/"{grb_name}"/"$grb_name"/g | sed s/"{gcnNo}"/"$gcnNo"/g | sed s/"{proposalNo}"/"$proposalNo"/g | sed s/"{pi}"/"$pi"/g | sed s/"{rev}"/"$rev"/g | sed s/"{grb_dateAndTime}"/"$grb_dateAndTime"/g | sed s/"{grb_ra}"/"$grb_ra"/g | sed s/"{grb_dec}"/"$grb_dec"/g | sed s/"{angDist}"/"$angDist"/g | sed s/"{pointingID}"/"$pointingID"/g | sed s/"{pointingRa}"/"$pointingRA"/g | sed s/"{pointingDec}"/"$pointingDec"/g | sed s/"{flux_val}"/"$flux_val"/g | sed "s/{dataRights}/$dataRights/" | sed s/"{note}"/"$note"/g > email_$1
