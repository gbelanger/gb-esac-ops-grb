...
if [[ `calc.pl nint\($flux_val\)` -lt 5 ]]
then
    dataRights="1240036 (Hanlon) will be granted data rights on all instruments."
else
    dataRights="1240028 (Gotz) will be granted data rights on IBIS, and 1240036 (Hanlon) will be granted data rights on the other instruments."
fi

note="Note that 1240030 (Savchenko) gets all IBIS Compton-mode data to study GRBs _outside_ the IBIS PCFOV (the PI is responsible for identifying the appropriate GRB, and inform ISOC), and 1240005 (Hurley) for the IPN program uses SPI-ACS data to correlate with other instruments (these data are public and do not require any observing time)."

sed s/"{source}"/"$source"/g email_template.txt | sed s/"{grb_name}"/"$grb_name"/g | sed s/"{gcnNo}"/"$gcnNo"/g | sed s/"{proposalNo}"/"$proposalNo"/g | sed s/"{pi}"/"$pi"/g | sed s/"{sourceName}"/"$source"/g | sed s/"{rev}"/"$rev"/g | sed s/"{grb_dateAndTime}"/"$grb_dateAndTime"/g | sed s/"{grb_ra}"/"$grb_ra"/g | sed s/"{grb_dec}"/"$grb_dec"/g | sed s/"{angDist}"/"$angDist"/g | sed s/"{pointingID}"/"$pointingID"/g | sed s/"{pointingRa}"/"$pointingRA"/g | sed s/"{pointingDec}"/"$pointingDec"/g | sed s/"{flux_val}"/"$flux_val"/g | sed s/"{dataRights}"/"$dataRights"/g | sed s/"{note}"/"$note"/g > email_$1
